(function() {
    ...
    $document.on('action.ready_trx_addons', function() {
        ...
        function trx_addons_woocommerce_add_to_cart_ajax(type) {
            ...
            if ($popup.length) {
                ...
                if ($bt.length) {
                    ...
                    $popup.find('.input-text.qty').on('change', function() {
                        $bt.attr('data-quantity', jQuery(this).val())
                    })
                }
            }
        }
        if (trx_addons_apply_filters('trx_addons_filter_ajax_add_to_cart_in_quick_view', !0)) {
        ...
        if (!0 || $body.hasClass('woocommerce') && $body.hasClass('archive')) {
            $document.on('action.init_hidden_elements', function(e, cont) {
                ...
                function check_available_variations($wrap) {
                    ...
                    $wrap.find('.trx_addons_product_attribute_item').each(function() {
                        ...
                        $attr.toggleClass('trx_addons_product_attribute_item_disabled', get_closest_variation(variations, attr_variation) === !1)
                    })
                }
        ...
        var reopen_after_reload = trx_addons_apply_filters('trx_addons_filter_reopen_filter_after_reload_products', !1),
            reopened = !1,
            reload_is_busy = !1,
            reload_is_allowed = !0,
            last_clicked_item = '',
        ...
        $document.on('action.init_hidden_elements', function() {
                ...
                search_forms.each(function() {
                    ...
                    form.on('submit', function(e) {
                        ...
                        if (type == 'filter') {
                            var $self = jQuery(this),
                                wrap = $self.parents('.sc_form_field_wrap');
                            wrap.find('.sc_form_field_item').toggleClass('sc_form_field_item_checked', !0);
                            var value = trx_addons_woocommerce_search_form_get_multiple_field_value(wrap);
                            wrap.find('.sc_form_field_param').val(value).trigger('change')
                        }
                        return !1
                        ...
                        if (type == 'filter') {
                            ...
                            $self.parents('.trx_addons_woocommerce_search_form_fields_wrap').toggleClass('trx_addons_woocommerce_search_form_fields_changed', !0);
                        ...
                        if (type == 'filter' && reload_is_allowed) {
                            if ($self.attr('name') == 'product_cat') {
                                ...
                                trx_addons_woocommerce_search_form_reload_products(form, $self, '.sc_form_field_param[name="product_cat"]', !0)
                        ...
                        if (type == 'filter') {
                            var $self = jQuery(this);
                            if (!$self.attr('disabled')) {
                                trx_addons_woocommerce_search_form_close_field($self.parents('.sc_form_field_opened'), !0)
                            }
                            e.preventDefault();
                            return !1
                        }
                    }).on('click', '.trx_addons_woocommerce_search_button_filters,' + '.trx_addons_woocommerce_search_button_show,' + '.trx_addons_woocommerce_search_close', function(e) {
                        var $self = jQuery(this),
                            applied = !1,
                        ...
                        if ($self.hasClass('trx_addons_woocommerce_search_button_filters') && !wrap.hasClass('trx_addons_woocommerce_search_form_fields_wrap_opened')) {
                            ...
                            if (wrap.hasClass('trx_addons_woocommerce_search_form_fields_wrap_opened') || ($self.parents('.sidebar').length && $self.parents('.trx_addons_woocommerce_search').hasClass('trx_addons_woocommerce_search_apply'))) {
                                if (wrap.data('param-changed') > 0) {
                                    var last_apply = last_opened_filter ? form.find('input[name="' + last_opened_filter + '"]').parents('.sc_form_field').find('.trx_addons_search_apply:not([disabled])') : !1;
                    ...
                    if (form.parents('.sidebar').length > 0) {
                        form.find('.sc_form_field_expanded').each(function() {
                            trx_addons_woocommerce_search_form_open_field(jQuery(this).removeClass('sc_form_field_expanded'), 'show', !1)
                        })
                    }
                    ...
                    var trx_addons_woocommerce_search_form_reload_products_after_timeout = trx_addons_throttle(trx_addons_woocommerce_search_form_reload_products, trx_addons_apply_filters('trx_addons_filter_reload_products_timeout', ajax ? 500 : 0), !0);
                    ...
                    }, trx_addons_apply_filters('trx_addons_filter_update_products_counter_timeout', 500), !0)
                })
            }
            ...
            function trx_addons_woocommerce_search_form_open_field(field, open_style, need_focus) {
                ...
                if (!1 && field.parents('.trx_addons_woocommerce_search').hasClass('trx_addons_woocommerce_search_apply')) {
            ...
            function trx_addons_woocommerce_search_form_param_changed(param) {
                ...
                if (selected.length && (wrap.parents('.trx_addons_woocommerce_tools').length === 0 || fields_wrap.hasClass('trx_addons_woocommerce_search_form_fields_wrap_opened'))) {
                    ...
                    for (var i = 0; i < parts.length; i++) {
                        ...
                        value_to_show += (value_to_show ? ', ' : '') + ($item.length && $item.text() == $item.html() ? $item.text() : trx_addons_proper(decodeURIComponent(parts[i])))
                    }
                ...
                jQuery.get(shop_url).done(function(response) {
                    ...
                    function list_products_replace(remove_loading) {
                        ...
                        if (new_products) {
                                ...
                                if (new_inline_css) {
                                    ...
                                    if (new_inline_block) {
                                        ...
                                        if (old_inline_block_start) {
                                            old_inline_css = old_inline_css.substring(0, old_inline_block_start) + new_inline_block + old_inline_css.substring(old_inline_block_end + inline_css_end.length);
                            ...
                            if (!remove_loading) {
                                jQuery(list_products_loading_selector).toggleClass('trx_addons_woocommerce_search_loading', !0).append(list_products_loading_html)
                            }
        ...
        }, trx_addons_apply_filters('trx_addons_filter_woocommerce_search_click_outside_timeout', 10), !0);
