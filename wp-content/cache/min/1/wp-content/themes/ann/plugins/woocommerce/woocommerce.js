jQuery(document).on('action.ready_ann', function() {
    ...
    jQuery('.woocommerce,.woocommerce-page').on('click', '.ann_shop_mode_buttons a', function(e) {
        ...
        ann_set_cookie('ann_shop_mode', mode, 365 * 24 * 60 * 60 * 1000);
    ...
    if (wishlist.length > 0 && compare.length > 0) {
        ...
        if (compare.hasClass('button')) {
            ...
            $document.on('ajaxComplete', function(e) {
                setTimeout(function() {
                    jQuery('.woocommerce .product .summary .yith-wcwl-add-to-wishlist .add_to_wishlist').eq(0).toggleClass('button', !0)
                }, 10)
            })
        }
    }
